    ...
    <tr className="border-b last:border-b-0 hover:bg-gray-50">
      <td className="py-4 px-4">
        <div className="flex items-center gap-2">
          <div>
            <div className="font-medium text-sm md:text-md line-clamp-1">{task.name}</div>
      ...
      <td className="py-4 px-4">
        <div className="flex -space-x-2">
          {[...Array(Math.min(3, task.noUsers))].map((_, i) => (
            <img
              key={i}
      ...
      <td className="py-4 px-4">
        <ProgressBar progress={task.progress || 0} /> {/* Assuming task has a progress field */}
      </td>
